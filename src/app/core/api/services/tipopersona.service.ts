...
export class TipoPersonaService implements BaseApiService<TipoPersona> {
  ...
  getAll(): Observable<TipoPersona[]> {
    return this.http
      .get<TipoPersona[]>(this.apiUrl)
      .pipe(catchError(this.handleError<TipoPersona[]>("getAll", [])));
  }
  ...
  getById(id: string): Observable<TipoPersona | null> {
    return this.http
      .get<TipoPersona>(`${this.apiUrl}/${id}`)
      .pipe(catchError(this.handleError<TipoPersona>(`getById id=${id}`)));
  }
  ...
  create(TipoPersona: TipoPersona): Observable<TipoPersona> {
    return this.http
      .post<TipoPersona>(this.apiUrl, TipoPersona, this.httpOptions)
      .pipe(catchError(this.handleError<TipoPersona>("create")));
  }
  ...
  update(id: string, TipoPersona: TipoPersona): Observable<boolean> {
    return this.http
      .put(`${this.apiUrl}/${id}`, TipoPersona, this.httpOptions)
      .pipe(
        map(() => true),
